...
| `WG_UDP_PORT_RANGE`           | *(unset)*         | `51820-52100`                  | **Docker Compose:** publish a **UDP port range** (`start-end`, same inside the container). If unset, only `WG_PORT` is published. **Also set in `.env` for the app:** new tunnel listen ports must fall in this range (or equal `WG_PORT` when unset), or creation/API/restore fails. |
