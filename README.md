...
* **Web UI session API** (same browser session cookie / password gate as other `/api/*` routes, no `X-API-Key`):
  - `GET /api/wireguard/tunnel` — array of tunnel summaries (`name`, `listen_port`, `address`, `peer_count`, Amnezia `config`, `tunnel_enabled`, `vpn_globally_enabled`, `running`, …).
  ...
  - `POST /api/wireguard/tunnels/add` — JSON `{ "tunnel": { … }, "overwrite": false }` for add/merge payloads (interface `name`, optional `listenport`, optional `addresses`, optional `enabled` to set per-tunnel `tunnelEnabled` in `{interface}.json`).
  - `POST /api/wireguard/tunnels/reset` — JSON `{ "tunnel": "wg0" }` to regenerate Amnezia obfuscation parameters for that interface.
  - `PUT /api/wireguard/tunnels/:tunnel/enabled` — JSON `{ "enabled": true|false }` to turn that interface on or off (`wg-quick` up/down; stored as `tunnelEnabled` in that tunnel’s JSON).
  ...
  - `GET /api/server-settings` / `PUT /api/server-settings` — read or update **runtime** overrides stored in `{WG_PATH}/wg-easy-server-settings.json` (optional keys: `wgHost`, `wgDefaultDns`, `vpnEnabled`, `compatApiEnabled`). Omitted keys follow `.env` (`WG_HOST`, `WG_DEFAULT_DNS`, `AMNEZIA_VPN_ENABLED`, `AMNEZIA_API_ENABLED`). The **Settings** tab can toggle **all** WireGuard interfaces, the pfSense-compatible API, and the public endpoint host and client DNS line without editing the container environment.
